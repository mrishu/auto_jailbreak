...
1. Click on RESET to reset the environment. Note that this does not change the current task, but will reset the `step_count` in State to 0 and delete the Target's chat history.
...
3. You can use the checkbox for "Delete Target Prev Chat" to delete the Target's history. This does not change the task, and can be used to start the conversation anew.
